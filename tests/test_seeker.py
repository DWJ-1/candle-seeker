"""Tests for CandleSeeker corpus crawler and classifier."""

from seeker.crawler import (
    CorpusCrawler,
    CorpusDocument,
    LanguageDetector,
    SeekerConfig,
...
from seeker.classifier import DomainClassifier, DomainLabel, ClassificationResult
from seeker.tokenizer import ChineseTokenizer, TokenizedDocument, compute_corpus_statistics
...
class TestDomainClassifier:
    def test_tech_classification(self):
        ...
        assert result.label == DomainLabel.TECHNOLOGY
    ...
    def test_literature_classification(self):
        ...
        result = classifier.classify(doc)
        assert result.label in (DomainLabel.LITERATURE, DomainLabel.GENERAL)
    ...
    def test_distribution(self):
        classifier = DomainClassifier()
        results = [
            ClassificationResult(DomainLabel.TECHNOLOGY, 0.8),
            ClassificationResult(DomainLabel.TECHNOLOGY, 0.7),
            ClassificationResult(DomainLabel.LITERATURE, 0.9),
        ]
        dist = classifier.distribution(results)
        assert dist["tech"] == 2
